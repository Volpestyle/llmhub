...
func (h *Kit) GenerateSpeech(ctx context.Context, in SpeechGenerateInput) (SpeechGenerateOutput, error) {
	if entitlement := h.entitlementForProvider(in.Provider); entitlement != nil {
		return h.GenerateSpeechWithContext(ctx, entitlement, in)
	}
	adapter, ok := h.adapters[in.Provider]
	if !ok {
		return SpeechGenerateOutput{}, fmt.Errorf("provider %s is not configured", in.Provider)
	}
...
func (h *Kit) GenerateSpeechWithContext(ctx context.Context, entitlement *EntitlementContext, in SpeechGenerateInput) (SpeechGenerateOutput, error) {
	adapter, err := h.factory(in.Provider, entitlement)
	if err != nil {
		return SpeechGenerateOutput{}, err
	}
